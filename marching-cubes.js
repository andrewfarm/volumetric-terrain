// lookup tables from https://github.com/BorisTheBrave/mc-dc/blob/a165b326849d8814fb03c963ad33a9faf6cc6dea/marching_cubes_3d.py
...
function marchingCubes(f, grid) {
    ...
    const cellSizeZ = (grid.maxZ - grid.minZ) / grid.numCellsZ;
    
    var mesh = [];
    
    let cellZ = grid.minZ;
    for (let z = 0; z < grid.numCellsZ; z++) {
        let cellY = grid.minY;
        for (let y = 0; y < grid.numCellsY; y++) {
            let cellX = grid.minX;
            for (let x = 0; x < grid.numCellsX; x++) {
                ...
                let triangles = CASES[caseNumber];
